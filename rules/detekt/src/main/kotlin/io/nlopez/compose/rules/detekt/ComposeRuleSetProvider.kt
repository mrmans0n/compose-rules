...
class ComposeRuleSetProvider : RuleSetProvider {
    ...
    override fun instance(): RuleSet = RuleSet(
        ...
        mapOf(
            ...
            RuleName("MultipleContentEmitters") to { config: Config -> MultipleContentEmittersCheck(config) },
            RuleName("MutableParameters") to { config: Config -> MutableParametersCheck(config) },
            RuleName("MutableStateAutoboxing") to { config: Config -> MutableStateAutoboxingCheck(config) },
            RuleName("MutableStateParameter") to { config: Config -> MutableStateParameterCheck(config) },
            RuleName("Naming") to { config: Config -> NamingCheck(config) },
            RuleName("ParameterNaming") to { config: Config -> ParameterNamingCheck(config) },
            RuleName("ParameterOrder") to { config: Config -> ParameterOrderCheck(config) },
            ...
            RuleName("RememberStateMissing") to { config: Config -> RememberStateMissingCheck(config) },
